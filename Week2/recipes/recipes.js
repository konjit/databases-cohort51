...
const GET_ALL_RECIPES = `SELECT name from recipes;`;

const recipes = await executeQuery(GET_ALL_RECIPES);
...
for (const { recipe } of recipes) {
    const ingredients = await listIngredients(recipe);
    const steps = await listSteps(recipe);
    ...
    console.log(`\nSteps for ${recipe}:`);
    steps.sort((a, b) => a.orders - b.orders).forEach(step => {
